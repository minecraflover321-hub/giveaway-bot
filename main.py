...
async def announce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("Usage: /announce <winner_count> <text>")
        return
    try:
...
async def end_giveaway(context):
    ...
    if not winners:
        giveaway_active = False
        return
    # Fetch reward (single reward for all winners)
    ...
    conn.commit()
    # Announce in channel and to owner
    ...
    cursor.execute(
        ...
        (",".join([u[1] for u in winners]), len(users), datetime.now().strftime("%Y-%m-%d %H:%M"))
    )
    conn.commit()
    giveaway_active = False
...
async def main():
    ...
    await app.start()
    await app.updater.start_polling()  # Polling async
    print("Bot started!")
